...
namespace wf13_bookrentalshop
{
    public partial class FrmGenre : Form
    {
        ...
        #region  <이벤트 핸들러 - 파라미터가 object sender, EventArgs e // 우리가 만든게 아님 >
        ...
        private void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            SaveData();         // 데이터 저장 / 수정 
            RefreshData();    // 데이터 재 조회 
            ClearInputs();    // 입력 창 클리어 
        }
        ...
        #region  < 커스텀 메서드- 파라미터가 마음대로 // 매개변수가 있을수도 없을수도 - 우리가 만든것들 대부분>

        private void RefreshData()
        {
            // DB divtbl에서 데이터 조회 & DgvResult에 뿌림 
            try
            {
                ...
                using (MySqlConnection conn = new MySqlConnection(Helpers.Commons.ConnString))
                {
                    ...
                    adapter.Fill(ds, "divtbl");   // divtbl 이름으로도 DataSet 접근 가능 

                    DgvResult.DataSource = ds.Tables[0];    //위아래 둘다 가능 
                    //DgvResult.DataSource = ds;
                    //DgvResult.DataMember = "divtbl";

                    DgvResult.Columns[0].HeaderText = "장르 코드";
                    DgvResult.Columns[1].HeaderText = "장르 명";
                }
            }
            catch (Exception ex)
            {
        ...
        private void ClearInputs()
        {
            ...
            TxtDivision.ReadOnly = false;  //  신규일때는 입력이 가능하게 해야함 !
            TxtDivision.Focus();
            isNew = true;  // 신규
        }
        ...
        private bool CheckValidation()
        {
            ...
            {
                result = false;
                errorMsg += "● 장르 코드을 입력하세요. \r\n";
            }
            if (string.IsNullOrEmpty(TxtNames.Text))
            {
                result = false;
                errorMsg += "● 장르 명을 입력하세요. \r\n";
            }
            if (result == false)
            {
